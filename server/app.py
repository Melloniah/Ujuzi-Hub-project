...
class FundiResource(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            db.session.commit()
            response = make_response(new_fundi.to_dict(rules=('-password_hash',)), 201)
            return response
        except Exception as e:
            return {"errors": "422: Unprocessable Entity", "message": str(e)}, 422
...
class FundiByID(Resource):
    ...
    def patch(self, id):
        ...
        if not fundi:
            return {"error": "Fundi not found."}, 404
        ALLOWED_FIELDS = {'name', 'price', 'phone_number', 'email', 'service_id', 'county_id'}
        for attr in data:
            if attr == 'password':
...
class FundiReviewsByID(Resource):
    def get(self, id):
        fundi = Fundi.query.filter_by(id=id).first()
        if not fundi:
            return {"error": "Fundi not found."}, 404
